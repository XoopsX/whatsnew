<?php
// $Id: d3forum_080_1.inc.php,v 1.1 2011/12/30 21:45:47 ohwada Exp $
...
if (! function_exists('d3forum_whatsnew_base')) {
	function d3forum_whatsnew_base( $mydirname, $category_option, $topic_option, $limit=0, $offset=0 )
	{
		...
		while ($row = $db->fetchArray($result)) 
		{
			...
			$ret[$i]['time']  = intval( $row['post_time'] );
			$ret[$i]['hits'] = intval( $row['topic_views'] );
			$ret[$i]['replies'] = intval( $row['topic_posts_count'] ) - 1;
			$ret[$i]['uid']  = intval( $row['uid'] );
			$ret[$i]['guest_name']  = $myts->makeTboxData4Show( $row['guest_name'] );
			$ret[$i]['id']  = $topic_id;
