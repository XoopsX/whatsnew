<?php
// $Id: nmblog_150.inc.php,v 1.1 2011/12/30 21:45:50 ohwada Exp $
...
//		$sql = sprintf('%s INNER JOIN %s as c on a.category_id = c.id left OUTER JOIN %s as m on a.id = m.article_id GROUP BY a.id ORDER BY post_date DESC', $sql, $xoopsDB->prefix('nmBlogCategory'),$xoopsDB->prefix('nmBlogComment'));
		...
		$sql = sprintf('SELECT a.id, a.uid, c.name, a.time, a.title, a.content, a.category_id, count( m.article_id ) as cnum FROM %s as a ', $xoopsDB->prefix('nmBlogArticle'));
