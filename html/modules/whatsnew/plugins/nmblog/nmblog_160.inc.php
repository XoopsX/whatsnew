<?php
// $Id: nmblog_160.inc.php,v 1.1 2011/12/30 21:45:50 ohwada Exp $

// 2009-03-06 K.OHWADA
// support MySQL 4.1
// mysql's UNIX_TIMESTAMP -> PHP's strtotime

//================================================================
// What's New Module
// get aritciles from module
// nmBlog 1.6rc <http://www.nm-eternity.net/~nerk/>
// 2008-01-14 K.Naramoto <http://www.tokoharu.jp/>
//================================================================
...
// http://www.tokoharu.jp/
// for What's New Module v2.4 & nmBlog v1.6RC3  
...
//		$sql = sprintf('SELECT a.id, a.uid, c.name, UNIX_TIMESTAMP(a.time) AS post_date, a.title, a.content, a.category_id, count( m.article_id ) as cnum, a.view FROM %s as a ', $xoopsDB->prefix('nmBlogArticle'));
//		$sql = sprintf('%s INNER JOIN %s as c on a.category_id = c.id left OUTER JOIN %s as m on a.id = m.article_id GROUP BY a.id ORDER BY post_date DESC', $sql, $xoopsDB->prefix('nmBlogCategory'),$xoopsDB->prefix('nmBlogComment'));
		...
		$sql = sprintf('SELECT a.id, a.uid, c.name, a.time, a.title, a.content, a.category_id, count( m.article_id ) as cnum, a.view FROM %s as a ', $xoopsDB->prefix('nmBlogArticle'));
		$sql = sprintf('%s INNER JOIN %s as c on a.category_id = c.id left OUTER JOIN %s as m on a.id = m.article_id GROUP BY a.id ORDER BY a.time DESC', $sql, $xoopsDB->prefix('nmBlogCategory'),$xoopsDB->prefix('nmBlogComment'));
		...
		while ($myrow=$xoopsDB->fetchArray($result)) {
			...
			$ret[$i]['hits']     = $myrow['view'];

//			$ret[$i]['time']  = $myrow['post_date'] + $TZ;
			$ret[$i]['time']  = strtotime( $myrow['time'] ) ;
