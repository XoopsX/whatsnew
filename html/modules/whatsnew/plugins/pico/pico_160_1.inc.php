<?php
// $Id: pico_160_1.inc.php,v 1.1 2011/12/30 21:45:49 ohwada Exp $
...
if (! function_exists('pico_whatsnew_base')) {
	function pico_whatsnew_base( $mydirname, $category_option, $limit=0, $offset=0 )
	{
		...
		$whr_readcontent = 'o.`cat_id` IN (' . implode( "," , pico_whatsnew_categories_can_read( $mydirname , $uid ) ) . ')' ;
		...
		$sql = "SELECT  *  FROM ".$db->prefix($mydirname."_contents")." o LEFT JOIN ".$db->prefix($mydirname."_categories")." c ON o.cat_id=c.cat_id WHERE ($whr_readcontent) AND ($whr_categories) AND o.visible AND o.created_time <= UNIX_TIMESTAMP() ORDER BY o.modified_time DESC" ;
		//$sql = "SELECT  o.content_id, o.vpath, o.cat_id, o.created_time, o.modified_time, o.poster_uid, o.subject, o.visible, o.use_cache, o.allow_comment, o.viewed, o.body, o.body_cached, o.filters, c.cat_id, c.cat_title FROM ".$db->prefix($mydirname."_contents")." o LEFT JOIN ".$db->prefix($mydirname."_categories")." c ON o.cat_id=c.cat_id WHERE ($whr_readcontent) AND ($whr_categories) AND o.visible AND o.created_time <= UNIX_TIMESTAMP() ORDER BY o.modified_time DESC" ;
...
// replies
			if( $replies && $row['allow_comment']) {
				$ret[$i]['replies'] = $row['comments_count'];
			}
...
// description
			...
			if ( $permissions[ $cat_id ]['can_readfull'] ){
...
if (! function_exists('pico_whatsnew_categories_can_read')) {
	function pico_whatsnew_categories_can_read( $mydirname , $uid )
	{
		global $xoopsUser ;
