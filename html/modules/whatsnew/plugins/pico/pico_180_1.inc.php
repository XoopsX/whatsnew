<?php
// $Id: pico_180_1.inc.php,v 1.1 2011/12/30 21:45:49 ohwada Exp $

// === option begin ===
$category_option = '';
//ɽ�����륫�ƥ��꡼�ֹ�򥫥��(,)�Ƕ��ڤäƵ���������ʤ������ƥ��꡼ɽ����
// --- option end ---
...
// 2009-06-13 photosite
// change pico_whatsnew_categories_can_read,pico_whatsnew_view_detail

// 2009-06-03 photosite
// check by category_permissions for pico 1.80

// 2008-09-30 photosite
...
if (! function_exists('pico_whatsnew_view_detail')) {
	function pico_whatsnew_view_detail( $mydirname , $uid , $cat_id )
	{
		...
		$sql = "SELECT cp.cat_id,cp.permissions FROM ".$db->prefix($mydirname."_category_permissions")." cp LEFT JOIN ".$db->prefix($mydirname."_categories")." c ON cp.cat_id=c.cat_permission_id  WHERE ($whr) AND c.cat_id=".$cat_id ."" ;
		$result = $db->query( $sql ) ;
		if( $result ) while( list( $cid , $serialized_permissions ) = $db->fetchRow( $result ) ) {
